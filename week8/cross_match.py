...
def leg_query(uf, path, ra_f, dec_f, n, radius, col_nams):
    ...
    # AJF add ras and decs to col_nams for legacy
    other_nams = ['RA', 'DEC'] 
    ...
    for f in tqdm(uf):
        ...
        # AJF keep only needed columns in each table read in to reduce size of table and thus size of all_tabs list of tables
    ...
    # AJF sort by input number
    final_table.sort('Input Survey Row Number')
    ...
    # AJF print time needed to completely create final table
    tt = end - start
...
def main():# AJF executes this section first (highest 'shell' of code)
    ...
    # AJF print percentages and number of objects
    print(f'The percent of objects matched from the legacy survey is: {perc_list[0]:.3g}%; the percent of objects matched from the input survey is {(((num_list[0])/(n))*100):.3g}% ({num_list[0]} / {n}).\n')
